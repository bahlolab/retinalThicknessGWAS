...
outDir=/vast/scratch/users/jackson.v/retThickness/GWASfollowup/pixelWiseResultsAllSentinels/results/
...
mkdir -p chr${chr}
rsync -av ${sentinelsFileDir}/allSigSNPs.txt chr${chr} 
...
do
  ...
  read pixel y x < <(sed -n ${pix}p ${slice}_pixels.txt)
    ...
    ref=${outDir}/chr${chr}/allSigSNPs.txt 
