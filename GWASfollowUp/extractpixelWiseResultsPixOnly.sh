...
outDir=/vast/scratch/users/jackson.v/retThickness/GWASfollowup/pixelWiseResultsPixOnlysentinels/results/
sentinelsFileDir=/wehisan/bioinf/lab_bahlo/projects/misc/retinalThickness/misc/
...
mkdir -p chr${chr}
rsync -av ${sentinelsFileDir}/pixelOnlySNPs.txt chr${chr} 
...
do
  ...
  read pixel y x < <(sed -n ${pix}p ${slice}_pixels.txt)
    ...
    ref=${outDir}/chr${chr}/pixelOnlySNPs.txt 
