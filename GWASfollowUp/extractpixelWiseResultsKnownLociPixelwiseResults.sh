...
outDir=/vast/scratch/users/jackson.v/retThickness/GWASfollowup/pixelWiseResultsKnownLoci/results/
...
mkdir -p chr${chr}
rsync -av ${sentinelsFileDir}/allKnownSNPs.txt chr${chr} 
...
do
  ...
  read pixel y x < <(sed -n ${pix}p ${slice}_pixels.txt)
    ...
    ref=${outDir}/chr${chr}/allKnownSNPs.txt 
