...
imputedScans <- lapply(c(1:10), function(chunk) {
  paste0("/wehisan/bioinf/lab_bahlo/projects/misc/retinalThickness/phenotypeCleaning/output/scansRefErrAdjustedImputedWideFormat_chunk",chunk,".csv") %>%
  fread
  }) %>% rbindlist
...
phenoAveraged <- imputedScans[, nScans := .N, by = "patID"] %>%
                 ...
                 .[, scan := NULL] %>%
                 .[, refErr := NULL] %>%
                 .[, lapply(.SD, mean, na.rm=TRUE), .SDcols = pixels, by = c("patID", "eye", "visit")]
...
fwrite(phenoOut, file = "/wehisan/bioinf/lab_bahlo/projects/misc/retinalThickness/phenotypeCleaning/processedData/scansRefErrAdjustedFinal.csv", sep = ",")
